...
def bandpass_filter(signal, lowcut=5.0, highcut=40.0, fs=128, order=4):
    """Bandpass filter for ECG (to emphasize QRS complex)."""
    nyq = 0.5 * fs
    b, a = butter(order, [lowcut / nyq, highcut / nyq], btype='band')
    return filtfilt(b, a, signal)

def extract_features(segment):
    """Compute 35 ECG + statistical features from 128-sample ECG segment."""
    features = {}
    ...
    features['avg_heart_rate'] = beat_count * 60  # BPM for 1 second
...
for idx, file in enumerate(sorted(csv_files), start=1):
    ...
    for i in range(0, len(values), SFREQ):
        segment = values[i:i + SFREQ]
        if len(segment) < SFREQ:
            break
        label = 1 if np.any(labels[i:i + SFREQ] == 1) else 0
    ...
    # Save
    output_name = file.replace(".csv", "_feature.csv")
